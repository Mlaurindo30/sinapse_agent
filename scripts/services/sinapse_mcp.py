...
TOOLS = [
    ...
    {
        "name": "sinapse_health",
        "description": "Health check of all Sinapse read-backends (UMC, NeuralMemory, sqlite-vec, claude-mem, Graphify, Graphiti, filesystem — the 7 backends fused by sinapse_query) and vault status. RTK (rtk-ai/rtk) is an unrelated CLI proxy for shell commands — it is NOT a read-backend and does NOT participate in sinapse_query; agents that want RTK install it themselves via 'rtk init --agent <name>'. Returns node count and backend availability.",
    ...
    },
    {
        ...
        "description": "Save an observation to claude-mem temporal memory. NOTE: only works when claude-mem is in server-beta mode (CLAUDE_MEM_RUNTIME=server-beta). In worker mode, observations are saved via hooks, not HTTP API. Falls back to saving in cerebro/cerebelo/padroes/Patterns.md as a temporal note.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "content": {
    ...
    },
    {
        ...
        "description": "Auto-partition a monolithic file or densely annotated content into individual conceptual atomic Zettelkasten notes under the anatomical temporal cortex using local Ollama model (qwen2.5-coder:3b). Updates knowledge graph automatically.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "source_file": {
    ...
    },
    {
        ...
        "description": "Capture exactly one screenshot on explicit agent/user request and save it to the vault (cerebro/cortex/parietal/inbox/visual/). Use only when visual context is necessary. Never use in loops or for monitoring. On multi-monitor setups, pass monitor explicitly.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "description": {
